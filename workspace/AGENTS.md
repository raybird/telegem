...
### 工具位置
```bash
node dist/tools/scheduler-cli.js
```
...
#### 1. 新增排程
```bash
node dist/tools/scheduler-cli.js add <名稱> <Cron表達式> <提示詞>
```

**範例**：
```bash
node dist/tools/scheduler-cli.js add "晨間報告" "0 9 * * *" "請生成今日市場分析報告"
```
...
#### 2. 列出所有排程
```bash
node dist/tools/scheduler-cli.js list
```

#### 3. 刪除排程
```bash
node dist/tools/scheduler-cli.js remove <ID>
```

**範例**：
```bash
node dist/tools/scheduler-cli.js remove 1
```
