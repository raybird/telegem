...
docker-compose run --rm moltbot jq --version
echo "✅ jq is installed"
echo ""

# Test 2: Check bash installation
echo "2️⃣ Testing bash availability..."
docker-compose run --rm moltbot bash --version | head -1
echo "✅ bash is available"
echo ""

# Test 3: Check .gemini directory and hooks
echo "3️⃣ Checking .gemini hooks..."
docker-compose run --rm moltbot ls -la /app/.gemini/hooks/
echo "✅ .gemini hooks are present"
echo ""

# Test 4: Check hook script permissions
echo "4️⃣ Verifying hook script permissions..."
docker-compose run --rm moltbot stat -c '%a %n' /app/.gemini/hooks/retrieve-memory.sh
echo "✅ retrieve-memory.sh is executable"
echo ""

# Test 5: Check environment variables
echo "5️⃣ Checking environment variables..."
docker-compose run --rm moltbot bash -c 'echo "GEMINI_PROJECT_DIR=$GEMINI_PROJECT_DIR"'
docker-compose run --rm moltbot bash -c 'echo "DB_DIR=$DB_DIR"'
docker-compose run --rm moltbot bash -c 'echo "DB_PATH=$DB_PATH"'
echo "✅ Environment variables are set"
echo ""

# Test 6: Test Hook script execution
echo "6️⃣ Testing retrieve-memory.sh Hook..."
docker-compose run --rm moltbot bash -c \
  'GEMINI_PROJECT_DIR=/app echo "{\"prompt\": \"Moltbot\"}" | /app/.gemini/hooks/retrieve-memory.sh'
echo "✅ Hook script executes successfully"
echo ""

# Test 7: Check volume mounting
echo "7️⃣ Verifying volume mounting..."
docker-compose run --rm moltbot ls -la /app/data/
